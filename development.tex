...
\item Word completion does not work in caption commands
\item If you have \verb+\begin{tabular}{ccc}+ and select the \verb+ccc+ the final \verb+\end{tabular}+ is replaced bz \verb+\end{ccc}+
\item if qmake and qmake-qt4 are in the path and qmake is qt3, the BUILD.sh script should call qmake-qt4
...
\begin{itemize}
	...
	\item if completing a filename, and the selected file has the same extension as the current filename, remove the existing extension
