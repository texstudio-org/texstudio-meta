...
\subsubsection{Bugs}
\begin{itemize}
\item continuous pdf mode scrollbar range calculation wrong (couldnot move to last pages before zooming)
\item bad scrolling in non-continuous modes
\item color command marked as unknown in beamer doc	
\item \verb+_+ marked as unknown in movie command
...
\begin{itemize}
	...
	\item Structure buttons does not have a pressed state. Hence, the user can not easily see which button is currently pressed, or active.
