...
\subsubsection{Bugs}
\begin{itemize}
\item keep indentation and white space addition
\item completion and enter \\dr... xxxxxxxxxxxxxx (jetzt habe ich schon wieder vergessen, was ein bugreport bedeutet)
...
\item no underscores \_ visible until zooming (Ubuntu 10.4, Deja Sans, qt 4.6.3)
...
\item line caches and cursor mirrors (create them with ctrl+alt, not visible at all)
...
\item completer list for references shows label defined in comments (e.g. \verb+\label{sssss}+)
\item \verb+22222\%+ is spell checked! Also \verb+9\%.+, but not \verb+9\%+
\item spellchecking destroys other formatting especially anoying in bib files or verbatim (simple workaround: don't spellcheck them | real solution: implement real merging of formats)
