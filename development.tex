...
\subsubsection{Bugs}
\begin{itemize}
\item If \verb+\only{text}<..>+ and \verb+\only<overlay specification>{text}<overlay specification>+ is in the list, writing \verb+\only<+ inserts former instead of latter completion
...
\item Restoration of the backslash of \verb+\}+ after completion doesn't work if the completion is canceled by an eof-character
