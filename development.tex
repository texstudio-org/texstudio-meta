...
\subsubsection{Bugs}
\begin{itemize}
\item unnecessary slow: (possible duplicate setHeight call after adjustWidth )
...
\item inline preview looks very strange (even overlapping, problem: no wrappign on empty lines) if it spans several lines (limit auto update to single line preview?) 
...
\item \verb+\left/\right+ not inverted in inverting parenthese selection
